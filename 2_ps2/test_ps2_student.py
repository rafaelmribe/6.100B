...
class InternalPs2Test(unittest.TestCase):
    ...
    def test_2_create_graph(self):
        ...
        for road in all_roads:
            ...
            if road_type != 'hill':
                ...
                possible_other_way_road_1 = DirectedRoad(road_end, road_start, road_travel_time*3, road_type, road_traffic_multiplier)
                possible_other_way_road_2 = DirectedRoad(road_end, road_start, road_travel_time*1/3, road_type, road_traffic_multiplier)
        ...
        expected_roads = {
            ...
            DirectedRoad(node2, node0, 7.0/3.0, HILL, 1.0),
            DirectedRoad(node1, node3, 6.0, HIGHWAY, 3.0),
            DirectedRoad(node3, node1, 6.0, HIGHWAY, 3.0),
            DirectedRoad(node3, node2, 18.0, HILL, 1.0),
            DirectedRoad(node2, node3, 6.0, HILL, 1.0),
            DirectedRoad(node4, node2, 7.0, HILL, 1.0),
            DirectedRoad(node2, node4, 7.0/3.0, HILL, 1.0),
            DirectedRoad(node3, node6, 5.0, LOCAL, 5.0),
            DirectedRoad(node6, node3, 5.0, LOCAL, 5.0),
            ...
            DirectedRoad(node6, node7, 7.0, DIRT, 2.0),
            DirectedRoad(node7, node6, 7.0, DIRT, 2.0),
            DirectedRoad(node7, node8, 2.0, HIGHWAY, 5.0),
            DirectedRoad(node8, node7, 2.0, HIGHWAY, 5.0),
            DirectedRoad(node8, node9, 10.0, DIRT, 2.0),
            DirectedRoad(node9, node8, 10.0, DIRT, 2.0),
    ...
    def test_3_path_multiple_roads_not_allowed(self):
        expected_path = [self.n1, self.n0, self.n2]
        expected_time = 22
        self._test_path(self.road_map, expected_path=expected_path, expected_time=expected_time,
                        message="Make sure your algorithm is finding the shortest path.")
    ...
    def test_4a_find_shortest_path_no_traffic(self):
        start, end = Node('N2'), Node('N9')
        expected_path = [Node('N2'), Node('N3'), Node('N6'), Node('N7'), Node('N9')]
        expected_time = 19.0
    ...
    def test_4b_find_shortest_path_restricted_avoid_local(self):
        start, end = Node('N8'), Node('N6')
        expected_path = [Node('N8'), Node('N7'), Node('N6')]
    ...
    def test_4b_find_shortest_path_restricted_avoid_both_local_hill(self):
        start, end = Node('N0'), Node('N6')
        expected_path = [Node('N0'), Node('N1'), Node('N3'), Node('N8'), Node('N7'), Node('N6')]
        student_path = find_shortest_path_restricted(ROADMAP_FILENAME, start, end)[0]
        self.assertEqual(list, type(student_path), "Make sure you are returning the correct type")
        self.assertEqual(expected_path, student_path, "Make sure your find_shortest_path_restricted is correct")
    ...
    def test_4c_find_shortest_path_in_traffic(self):
        ...
        expected_path = [Node('N0'), Node('N2'), Node('N3'), Node('N8'), Node('N7'), Node('N9')]
        expected_time = 39.0
