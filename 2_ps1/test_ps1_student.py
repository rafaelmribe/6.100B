...
def check_valid_mapping(election_states, move_voters_result, expected_results):
    ...
    orig_winner, orig_loser = ps1.get_election_winner(election_states)
    ...
    new_winner, new_loser = ps1.get_election_winner(election_copy)
...
class TestPS1(unittest.TestCase):
    def test_1_load_election_data(self):
        # 600_results.txt
        expected_parsed_election = [ps1.State("TX", 1, 2, 530), ps1.State("CA", 4, 5, 3), ps1.State("MA", 7, 8, 5)]
        parsed_election = ps1.load_election_data("600_results.txt")

        self.assertIsInstance(parsed_election, list, f"load_election_data should have returned a list, but instead returned an instance of {type(parsed_election)}.")
        self.assertEqual(len(parsed_election), len(expected_parsed_election), f"The length of the list returned by load_election_data should have been {len(expected_parsed_election)}, but found it returned {len(parsed_election)}.")
        self.assertTrue(all(isinstance(st, ps1.State) for st in parsed_election), f"All items in the list returned by load_election_data should have been State instances, but found your output was {parsed_election}.")
        ...
        parsed_election = ps1.load_election_data("6100A_results.txt")
    ...
    def test_2a_get_election_winner(self):
        rep_won = ("rep", "dem")
        dem_won = ("dem", "rep")
        ...
        winner = ps1.get_election_winner(parsed_election)
        ...
        # 2020_results.txt
        expected_winner = dem_won
        winner = ps1.get_election_winner(ps1.load_election_data("2020_results.txt"))
        ...
        winner = ps1.get_election_winner(ps1.load_election_data("2016_results.txt"))
        self.assertEqual(expected_winner, winner, f"For the 2016 election: expected {expected_winner}, got {winner}.")

        # 2012_results.txt
        expected_winner = dem_won
        winner = ps1.get_election_winner(ps1.load_election_data("2012_results.txt"))
    ...
    def test_2b_get_winning_states(self):
        ...
        winners = [state.get_name() for state in ps1.get_winning_states(ps1.load_election_data("600_results.txt"))]
        ...
        winners = [state.get_name() for state in ps1.get_winning_states(ps1.load_election_data("6100A_results.txt"))]
        self.assertIsInstance(winners, list, f"get_winning_states should have returned a list, but instead returned an instance of {type(winners)}.")
        self.assertEqual(len(winners), len(set(winners)), "get_winning_states should have returned a list with no duplicates, but found it had duplicates.")
        ...
        winners = [state.get_name() for state in ps1.get_winning_states(ps1.load_election_data("2020_results.txt"))]
        self.assertIsInstance(winners, list, f"get_winning_states should have returned a list, but instead returned an instance of {type(winners)}.")
        self.assertEqual(len(winners), len(set(winners)), "get_winning_states should have returned a list with no duplicates, but found it had duplicates.")
        ...
        winners = [state.get_name() for state in ps1.get_winning_states(ps1.load_election_data("2016_results.txt"))]
        self.assertIsInstance(winners, list, f"get_winning_states should have returned a list, but instead returned an instance of {type(winners)}.")
        self.assertEqual(len(winners), len(set(winners)), "get_winning_states should have returned a list with no duplicates, but found it had duplicates.")
        ...
        winners = [state.get_name() for state in ps1.get_winning_states(ps1.load_election_data("2012_results.txt"))]
        self.assertIsInstance(winners, list, f"get_winning_states should have returned a list, but instead returned an instance of {type(winners)}.")
        self.assertEqual(len(winners), len(set(winners)), "get_winning_states should have returned a list with no duplicates, but found it had duplicates.")
        self.assertEqual(expected_winners, set(winners), f"For the 2016 election: expected {expected_winners}, got {winners}.")

    def test_2c_ec_votes_needed_to_flip(self):
        # 600_results.txt
        expected_ecvotes = 270
        ecvotes = ps1.ec_votes_needed_to_flip(ps1.load_election_data("600_results.txt"))
        self.assertIsInstance(ecvotes, int, f"ec_votes_needed_to_flip should have returned an int, but instead returned an instance of {type(ecvotes)}.")
        self.assertEqual(expected_ecvotes, ecvotes, f"For the sample election: expected {expected_ecvotes}, got {ecvotes}.")

        # 6100A_results.txt
        expected_ecvotes = 2
        ecvotes = ps1.ec_votes_needed_to_flip(ps1.load_election_data("6100A_results.txt"), total=14)
        self.assertIsInstance(ecvotes, int, f"ec_votes_needed_to_flip should have returned an int, but instead returned an instance of {type(ecvotes)}.")
        self.assertEqual(expected_ecvotes, ecvotes, f"For the second sample election: expected {expected_ecvotes}, got {ecvotes}.")

        # 2020_results.txt
        expected_ecvotes = 38
        ecvotes = ps1.ec_votes_needed_to_flip(ps1.load_election_data("2020_results.txt"))
        self.assertIsInstance(ecvotes, int, f"ec_votes_needed_to_flip should have returned an int, but instead returned an instance of {type(ecvotes)}.")
        self.assertEqual(expected_ecvotes, ecvotes, f"For the 2020 election: expected {expected_ecvotes}, got {ecvotes}.")

        # 2016_results.txt
        expected_ecvotes = 37
        ecvotes = ps1.ec_votes_needed_to_flip(ps1.load_election_data("2016_results.txt"))
        self.assertIsInstance(ecvotes, int, f"ec_votes_needed_to_flip should have returned an int, but instead returned an instance of {type(ecvotes)}.")
        self.assertEqual(expected_ecvotes, ecvotes, f"For the 2016 election: expected {expected_ecvotes}, got {ecvotes}.")

        # 2012_results.txt
        expected_ecvotes = 64
        ecvotes = ps1.ec_votes_needed_to_flip(ps1.load_election_data("2012_results.txt"))
        self.assertIsInstance(ecvotes, int, f"ec_votes_needed_to_flip should have returned an int, but instead returned an instance of {type(ecvotes)}.")
    ...
    def test_3_brute_swing_states(self):
        ...
        election_states = ps1.load_election_data("600_results.txt")
        loser_lost_states = ps1.get_winning_states(election_states)
        loser_ecvotes_reqd = ps1.ec_votes_needed_to_flip(election_states)
        ...
        election_states = ps1.load_election_data("6100B_results.txt")
        loser_lost_states = ps1.get_winning_states(election_states)
        loser_ecvotes_reqd = ps1.ec_votes_needed_to_flip(election_states, total=14)
        raw_swing_states = ps1.brute_force_swing_states(copy.deepcopy(loser_lost_states), loser_ecvotes_reqd)
        swing_states, voters_moved = raw_swing_states
        swing_names = [state.get_name() for state in swing_states]
        actual_output = set(swing_names), voters_moved
        ...
        election_states = ps1.load_election_data("2020_results_brute.txt")
        loser_lost_states = ps1.get_winning_states(election_states)
        loser_ecvotes_reqd = ps1.ec_votes_needed_to_flip(election_states, total=122)
        raw_swing_states = ps1.brute_force_swing_states(copy.deepcopy(loser_lost_states), loser_ecvotes_reqd)
        swing_states, voters_moved = raw_swing_states
        swing_names = [state.get_name() for state in swing_states]
        actual_output = set(swing_names), voters_moved
        ...
        election_states = ps1.load_election_data("2016_results_brute.txt")
        loser_lost_states = ps1.get_winning_states(election_states)
        loser_ecvotes_reqd = ps1.ec_votes_needed_to_flip(election_states, total=44)
        raw_swing_states = ps1.brute_force_swing_states(copy.deepcopy(loser_lost_states), loser_ecvotes_reqd)
        swing_states, voters_moved = raw_swing_states
        swing_names = [state.get_name() for state in swing_states]
        actual_output = set(swing_names), voters_moved
        ...
        election_states = ps1.load_election_data("2012_results_brute.txt")
        loser_lost_states = ps1.get_winning_states(election_states)
        loser_ecvotes_reqd = ps1.ec_votes_needed_to_flip(election_states, total=98)
        raw_swing_states = ps1.brute_force_swing_states(copy.deepcopy(loser_lost_states), loser_ecvotes_reqd)
        swing_states, voters_moved = raw_swing_states
        swing_names = [state.get_name() for state in swing_states]
        actual_output = set(swing_names), voters_moved
    ...
    def test_4a_move_max_voters(self):
        ...
        loser_lost_states = ps1.get_winning_states(ps1.load_election_data("600_results.txt"))
        ...
        election_states = ps1.load_election_data("2020_results.txt")
        loser_lost_states = ps1.get_winning_states(election_states)
        loser_ecvotes_reqd = ps1.ec_votes_needed_to_flip(election_states)
        ...
        election_states = ps1.load_election_data("2016_results.txt")
        loser_lost_states = ps1.get_winning_states(election_states)
        loser_ecvotes_reqd = ps1.ec_votes_needed_to_flip(election_states)
        loser_ecvotes = sum(state.get_ecvotes() for state in loser_lost_states)
        raw_moved_max_voters = ps1.max_voters_moved(loser_lost_states, loser_ecvotes-loser_ecvotes_reqd)
        ...
        election_states = ps1.load_election_data("2012_results.txt")
        loser_lost_states = ps1.get_winning_states(election_states)
        loser_ecvotes_reqd = ps1.ec_votes_needed_to_flip(election_states)
        loser_ecvotes = sum(state.get_ecvotes() for state in loser_lost_states)
        raw_moved_max_voters = ps1.max_voters_moved(loser_lost_states, loser_ecvotes-loser_ecvotes_reqd)
