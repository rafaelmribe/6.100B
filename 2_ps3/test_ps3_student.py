...
class ps3_P1A(unittest.TestCase):
    ...
    def test_room_init(self):
        ...
        self.assertEqual(
            room.get_width(), width, "Room width was not initialized correctly"
        )
        self.assertEqual(
            room.get_height(), height, "Room height was not initialized correctly"
        )
        self.assertEqual(
            room.get_dust_amount(1, 1),
            dust_amount,
            "Room dust amount was not initialized correctly",
        )
    ...
    def test_clean_tile_at_position_PosToZero(self):
        ...
        for x, y in xyrange(width, height):
            room.clean_tile_at_position(
                ps3.Position(x + random.random(), y + random.random()), dust_amount
            )
    ...
    def test_clean_tile_at_position_PosToPos(self):
        ...
        for x, y in xyrange(width, height):
            room.clean_tile_at_position(
                ps3.Position(x + random.random(), y + random.random()), dust_amount - 1
            )
            # using random.random in case there is any issue with specific parts of a tile
        for x, y in xyrange(width, height):
            self.assertFalse(
    ...
    def test_clean_tile_at_position_ZeroToZero(self):
        ...
        for x, y in xyrange(width, height):
            room.clean_tile_at_position(
                ps3.Position(x + random.random(), y + random.random()), 1
            )
            # using random.random in case there is any issue with specific parts of a tile
        for x, y in xyrange(width, height):
            self.assertTrue(
                room.is_tile_cleaned(x, y),
                "Clean tile {} was marked clean, no negative dust allowed".format(
                    (x, y)
                ),
            )
            ...
            for y in [0.0, -0.1, height - 0.1, height, height + 0.1]:
                ...
                self.assertEqual(
                    ...
                    "position {},{} is incorrect: expected {}, got {}".format(
                        x, y, sols[count], room.is_position_in_room(pos)
                    ),
                )
                count += 1
...
class ps3_P1B(unittest.TestCase):
    ...
    def test_getset_position(self):
        """Test get_position and set_position"""
        ...
        positions = [
            ps3.Position(1.0, 1.0),
            ps3.Position(2.0, 2.0),
            ps3.Position(3.0, 3.0),
            ps3.Position(4.0, 4.0),
            ps3.Position(5.0, 5.0),
        ]
        for pos_index, robot in enumerate(robots):
            robot.set_position(positions[pos_index])
        for pos_index, robot in enumerate(robots):
            robot_pos = robot.get_position()
            self.assertEqual(
                robot_pos,
                positions[pos_index],
                "Robot position set or retrieved incorrectly: expected {}, got {}".format(
                    positions[pos_index], robot_pos
                ),
            )
...
class ps3_P2(unittest.TestCase):
    ...
    def test_update_position_and_cleanBasicRobot(self):
        ...
        for i in range(20):
            robot.update_position_and_clean()
            self.assertTrue(
                r.is_position_in_room(robot.get_position()),
...
class ps3_P5_Basic(SimulationTester):
    ...
    def testBasicSimulation1(self):
        "Test cleaning 100% of a 5x5 room"
        try:
            self.run_simulation(
                ((142, 174),), (1, 1.0, 1, 5, 5, 1, 1.0, 100, ps3.BasicRobot)
            )
        except:
            print("Unexpected error:", sys.exc_info()[1])
            raise

    def testBasicSimulation2(self):
        "Test cleaning 75% of a 10x10 room (Basic Robot)"
        self.run_simulation(
            ((183, 198),), (1, 1.0, 1, 10, 10, 1, 0.75, 100, ps3.BasicRobot)
        )

    def testBasicSimulation3(self):
        "Test cleaning 90% of a 10x10 room (Basic Robot)"
        self.run_simulation(
            ((298, 327),), (1, 1.0, 1, 10, 10, 1, 0.9, 100, ps3.BasicRobot)
        )

    def testBasicSimulation4(self):
        "Test multiple robots (95% of a 20x20 room with 5 robots (Basic Robot))"
        self.run_simulation(
            ((289, 303),), (5, 1.0, 1, 20, 20, 1, 0.95, 100, ps3.BasicRobot)
        )

    def testBasicSimulation5(self):
        "Test multiple robots and different speeds (90% of a 10x10 room with 3 robots of speed 0.5 (Basic Robot))"
        self.run_simulation(
            ((155, 180),), (3, 0.5, 1, 10, 10, 1, 0.9, 100, ps3.BasicRobot)
        )

    def testBasicSimulation6(self):
        "Test cleaning 100% of a 5x5 room (Basic Robot, 5 dust/tile, cleaning_volume = 3)"
        self.run_simulation(
            ((206, 266), (180, 240)), (1, 1.0, 3, 5, 5, 5, 1.0, 100, ps3.BasicRobot)
        )

    def testBasicSimulation7(self):
        "Test cleaning 100% of a 5x5 room (Basic Robot, 6 dust/tile, cleaning_volume = 3)"
        self.run_simulation(
            ((206, 266), (180, 240)), (1, 1.0, 3, 5, 5, 6, 1.0, 100, ps3.BasicRobot)
        )

    def testBasicSimulation8(self):
        "Test multiple robots (95% of a 10x10 room with 5 robots (Basic Robot)) cleaning_volume = 2, 6 dust/tile"
        self.run_simulation(
            ((137, 198), (130, 190)), (5, 1.0, 2, 10, 10, 6, 0.95, 100, ps3.BasicRobot)
        )

    def testBasicSimulation9(self):
        """Test multiple robots and different speeds (90% of a 5x5 room with 3 robots of speed 0.5
        (Basic Robot)), cleaning_volume = 2, 6 dust/tile"""
        self.run_simulation(
            ((48, 108), (45, 104)), (3, 0.5, 2, 5, 5, 6, 0.9, 100, ps3.BasicRobot)
        )
...
if __name__ == "__main__":
    ...
    suite.addTest(unittest.makeSuite(ps3_P2))
    suite.addTest(unittest.makeSuite(ps3_P5_Basic))
    SIMULATION_TIME_LIMIT = 100.0  # reset the time limit for the faulty robot
    suite.addTest(unittest.makeSuite(ps3_P5_Faulty))
    SIMULATION_TIME_LIMIT = 400.0  # reset the time limit for the smart robot
    suite.addTest(unittest.makeSuite(ps3_P5_Smart))
